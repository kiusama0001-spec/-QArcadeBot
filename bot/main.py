...
async def announce_record(game, user, score, season):
  ...
  name = html.escape(
      user["display_name"] or user["username"] or str(user["user_id"])
  )
  try:
...
async def show_top(m, game=None, title="TOP"):
  ...
  for i, r in enumerate(rows, 1):
    name = html.escape(
        r["display_name"] or r["username"] or str(r["user_id"])
    )
...
async def start(m):
  await m.answer(
      "🎮 <b>Q-ArcadeBot</b>\nEn el grupo escribe <code>@QArcadeBot</code> para"
      " abrir el catálogo."
  )
...
async def help_(m):
  await m.answer(
      "Inline: @QArcadeBot\nRankings: /top, /toppacman, /topmario, /toptetris,"
      " /topexcitebike, /topcontra, /topnaves, /topsunset, /topcarros,"
      " /topcircus, /toppokemon\nAdmin: /resetscores, /nuevaedicion"
  )
...
async def edition(m):
  ...
  new = db.new_season()
  await m.answer(
      f"🎮 Nueva edición iniciada. Temporada {old} cerrada; temporada {new}"
      " activa."
  )
